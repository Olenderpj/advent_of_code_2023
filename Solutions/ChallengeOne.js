...
const file = "../Input/ChallengeOne.txt";
  ...
  _.keys(ENUM_NUMBER).forEach((numKey) => {
    if (line.includes(numKey)) {
      ...
      line = line.replaceAll(
        numKey,
        `${numKey[0]}${ENUM_NUMBER[numKey]}${numKey[numKey.length - 1]}`
      );
    }
  });
  return line;
...
function partTwo() {
  ...
  lines.forEach((line) => {
    ...
    // first character + number + last character
    let value = `${stripLine[0]}${stripLine[stripLine.length - 1]}`;
    sum += Number(value);
  });
...
console.log("Day 1 - Challenge 1: ", c1, 'Challenge 2: ', c2)
